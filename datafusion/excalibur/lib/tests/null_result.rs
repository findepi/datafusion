...
fn test_invoke_array_with_nulls() {
   let udf = try_div_udf();
    ...
    let invoke_args = vec![
        ColumnarValue::Array(Arc::new(Int32Array::from(vec![Some(0), None, Some(15), Some(0), Some(3), Some(60)]))),
        ColumnarValue::Array(Arc::new(Int32Array::from(vec![Some(1), Some(3), None, Some(0), Some(0), Some(15)]))),
    ];
    let ColumnarValue::Array(result_array) = udf
        .invoke_with_args(ScalarFunctionArgs {
            args: invoke_args,
